...
- `GET /api/interview-records/{recordId}/review` exposes review provenance:
  ...
  - `reviewLaneSummary`
    ...
    - each lane also carries additive `primaryAction`, `primaryActionLabel`, `secondaryAction`, `secondaryActionLabel`, `emptyStateMessage`, `completionMessage`, and `blockingReasons[]` so lane cards can render CTA buttons, empty states, completion copy, and disabled-state explanations without duplicating backend rules
