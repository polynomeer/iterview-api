...
- `GET /api/interview-records/{recordId}/review` exposes review provenance:
  ...
  - `reviewLaneSummary`
    ...
    - each lane also carries additive `sortOrder`, `highlightVariant`, `badgeText`, `summaryText`, `recommendedTab`, `defaultExpanded`, `analyticsKey`, `trackingContext`, `helpText`, `whyItMatters`, `severity`, `highestPriority`, `primaryAction`, `primaryActionLabel`, `secondaryAction`, `secondaryActionLabel`, `emptyStateMessage`, `emptyStateCtaAction`, `emptyStateCtaLabel`, `completionMessage`, `completionCtaAction`, `completionCtaLabel`, and `blockingReasons[]` so lane cards can render CTA buttons, header copy, initial expansion state, initial tab focus, analytics payloads, helper copy, empty-state actions, completion actions, disabled-state explanations, and lane ordering without duplicating backend rules
