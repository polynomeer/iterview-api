...
- `GET /api/interview-records/{recordId}/review` exposes review provenance:
  ...
  - `reviewLaneSummary`
    ...
    - each lane also carries additive `sortOrder`, `highlightVariant`, `severity`, `highestPriority`, `primaryAction`, `primaryActionLabel`, `secondaryAction`, `secondaryActionLabel`, `emptyStateMessage`, `completionMessage`, and `blockingReasons[]` so lane cards can render CTA buttons, empty states, completion copy, disabled-state explanations, and lane ordering without duplicating backend rules
