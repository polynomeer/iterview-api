...
class InterviewRecordApiIntegrationTest {
    ...
    fun `upload interview record structures transcript and exposes review APIs`() {
        ...
        mockMvc.perform(get("/api/interview-records/$recordId/transcript").header("Authorization", authHeader))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.playback.playbackAvailable").value(true))
            .andExpect(jsonPath("$.playback.audioDurationMs").value(4000))
            .andExpect(jsonPath("$.playback.sessionRange.startMs").value(0))
            .andExpect(jsonPath("$.playback.sessionRange.endMs").value(3999))
            .andExpect(jsonPath("$.playback.sessionRange.startTimestampLabel").value("00:00"))
            .andExpect(jsonPath("$.playback.sessionRange.endTimestampLabel").value("00:03"))
            .andExpect(jsonPath("$.segments.length()").value(4))
            .andExpect(jsonPath("$.segments[0].timestampLabel").value("00:00"))
        ...
        val questions = mockMvc.perform(get("/api/interview-records/$recordId/questions").header("Authorization", authHeader))
            ...
            .andExpect(jsonPath("$.items[0].questionRange.startMs").value(0))
            .andExpect(jsonPath("$.items[0].answerRange.startMs").value(1000))
            .andExpect(jsonPath("$.items[0].questionAnswerRange.endMs").value(1999))
            .andExpect(jsonPath("$.items[0].answer.replayRange.startMs").value(1000))
            .andExpect(jsonPath("$.items[0].answer.strengthTags[0]").value("quantified"))
            .andExpect(jsonPath("$.items[1].questionRange.startMs").value(2000))
            .andExpect(jsonPath("$.items[1].questionAnswerRange.endMs").value(3999))
            .andReturn()
            .response
            .contentAsString
            .let(objectMapper::readTree)
        ...
        mockMvc.perform(get("/api/interview-records/$recordId/review").header("Authorization", authHeader))
            ...
            .andExpect(jsonPath("$.questionDistributionSummary.questionTypeCounts.general").value(1))
            .andExpect(jsonPath("$.questionDistributionSummary.questionTypeCounts.technical_depth").value(1))
            .andExpect(jsonPath("$.questionDistributionSummary.topicTagCounts.caching").value(2))
            .andExpect(jsonPath("$.questionOriginSummary.generalQuestions").value(0))
            .andExpect(jsonPath("$.questionOriginSummary.resumeLinkedQuestions").value(2))
            ...
            .andExpect(jsonPath("$.replayReadiness.recommendedReplayModeLabel").value("Original replay"))
            .andExpect(jsonPath("$.replayReadiness.statusBadgeText").value("Replay ready"))
            .andExpect(jsonPath("$.replayReadiness.statusVariant").value("success"))
            .andExpect(jsonPath("$.replayReadiness.statusSummary").value("Replay can start with the recommended interviewer pattern."))
            .andExpect(jsonPath("$.replayReadiness.primaryCtaLabel").value("Start replay"))
            .andExpect(jsonPath("$.replayReadiness.blockedCtaLabel").value("Resolve replay blockers"))
            .andExpect(jsonPath("$.replayReadiness.blockers.length()").value(0))
            .andExpect(jsonPath("$.replayReadiness.blockerDetails.length()").value(0))
            ...
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.blockingReasonDetails.length()").value(0))
            ...
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].label").value("Weak answers present"))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].severity").value("medium"))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].priority").value("p1"))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].highlightVariant").value("warning"))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].sortOrder").value(1))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].recommendedAction").value("review_answers"))
            .andExpect(jsonPath("$.reviewLaneSummary.question.blockingReasonDetails[0].recommendedActionTarget").value("question:questions"))
            ...
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].label").value("Weak follow-up threads"))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].severity").value("medium"))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].priority").value("p1"))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].highlightVariant").value("warning"))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].sortOrder").value(1))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].recommendedAction").value("review_weak_chain"))
            .andExpect(jsonPath("$.reviewLaneSummary.thread.blockingReasonDetails[0].recommendedActionTarget").value("thread:weak_threads"))
            ...
            .andExpect(jsonPath("$.answerQualitySummary.quantifiedAnswerCount").value(1))
            .andExpect(jsonPath("$.answerQualitySummary.structuredAnswerCount").value(0))
            .andExpect(jsonPath("$.answerQualitySummary.tradeoffAwareAnswerCount").value(1))
            ...
            .andExpect(jsonPath("$.timelineNavigation.items[0].questionRange.startMs").value(0))
            .andExpect(jsonPath("$.timelineNavigation.items[0].answerRange.startMs").value(1000))
            .andExpect(jsonPath("$.timelineNavigation.items[0].questionAnswerRange.endMs").value(1999))
            ...
            .andExpect(jsonPath("$.actionRecommendations.availableActionLabels.confirm").value("Confirm review"))
            ...
            .andExpect(jsonPath("$.actionRecommendations.blockingReasonDetails.length()").value(0))
            ...
            .andExpect(jsonPath("$.questionSummaries[0].originType").value("resume_linked"))
            .andExpect(jsonPath("$.questionSummaries[0].isFollowUp").value(false))
            .andExpect(jsonPath("$.questionSummaries[0].questionRange.startMs").value(0))
            .andExpect(jsonPath("$.questionSummaries[0].answerRange.startMs").value(1000))
            .andExpect(jsonPath("$.questionSummaries[0].questionAnswerRange.endMs").value(1999))
            ...
            .andExpect(jsonPath("$.followUpThreads[0].quantifiedQuestionCount").value(1))
            .andExpect(jsonPath("$.followUpThreads[0].recommendedAction").value("review_weak_chain"))
            .andExpect(jsonPath("$.followUpThreads[0].threadRange.startMs").value(0))
            .andExpect(jsonPath("$.followUpThreads[0].threadRange.endMs").value(3999))
            ...
            .andExpect(jsonPath("$.followUpThreads[0].tradeoffAwareQuestionCount").value(1))
        ...
        mockMvc.perform(get("/api/interview-records/$recordId/review").header("Authorization", authHeader))
            ...
            .andExpect(jsonPath("$.questionDistributionSummary.questionTypeCounts").isMap)
            .andExpect(jsonPath("$.questionOriginSummary.generalQuestions").value(0))
            .andExpect(jsonPath("$.replayReadiness.ready").value(true))
            .andExpect(jsonPath("$.replayReadiness.statusBadgeText").value("Replay ready"))
            .andExpect(jsonPath("$.transcriptIssueSummary.confirmedTextOverrideCount").value(0))
    ...
    @Test
    fun `interviewer profile endpoint returns conflict when analysis is pending without structured questions`() {
        val audio = MockMultipartFile("file", "real-interview.wav", "audio/wav", "fake-audio".toByteArray())
        val created = mockMvc.perform(
            multipart("/api/interview-records")
                .file(audio)
    ...
    fun `interviewer profile endpoint backfills profile when analysis status is stale`() {
        ...
        val recordId = created["id"].asLong()
        jdbcTemplate.update("DELETE FROM interviewer_profiles WHERE source_interview_record_id = ?", recordId)
        jdbcTemplate.update("UPDATE interview_records SET analysis_status = 'pending' WHERE id = ?", recordId)
    ...
    fun `review exposes transcript issue summary for pending transcript overrides`() {
        ...
        mockMvc.perform(get("/api/interview-records/$recordId/review").header("Authorization", authHeader))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.questionSourceCounts.confirmed").value(1))
            .andExpect(jsonPath("$.answerSourceCounts.confirmed").value(1))
            .andExpect(jsonPath("$.transcriptIssueSummary.speakerOverrideSegmentCount").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.speakerOverrideSegmentSequences.length()").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.confirmedTextOverrideCount").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.editedSegmentSequences.length()").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.resolvedIssueCount").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.unresolvedIssueCount").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.confirmationReadiness").value("ready"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.totalCount").value(0))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.sortOrder").value(3))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.highlightVariant").value("neutral"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.badgeText").value("Ready"))
            ...
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.defaultExpanded").value(false))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.analyticsKey").value("practical_review_lane_transcript"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.trackingContext.highlightVariant").value("neutral"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.accessibilityLabel").value("Transcript review lane"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.screenReaderSummary").value("Transcript lane is ready."))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.needsReviewCount").value(0))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.readiness").value("ready"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.severity").value("low"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.highestPriority").value("p2"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.primaryAction").value("confirm"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.primaryActionLabel").value("Confirm review"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.primaryActionTarget").value("review:confirm"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.primaryActionTargetPayload.focus").value("confirmation_cta"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.secondaryAction").value("start_replay"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.secondaryActionTarget").value("review:replay"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.secondaryActionTargetPayload.panel").value("replay_launch"))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.emptyStateMessage").value("No transcript issues detected."))
            ...
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.blockingReasons.length()").value(0))
            .andExpect(jsonPath("$.reviewLaneSummary.transcript.blockingReasonDetails.length()").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.reviewerLaneCounts").isMap)
            .andExpect(jsonPath("$.transcriptIssueSummary.topPrioritySegmentActions.length()").value(0))
            .andExpect(jsonPath("$.transcriptIssueSummary.segmentActions.length()").value(0))
            .andExpect(jsonPath("$.replayReadiness.statusBadgeText").value("Replay ready"))
            .andExpect(jsonPath("$.replayReadiness.statusVariant").value("success"))
            .andExpect(jsonPath("$.replayReadiness.statusSummary").value("Replay can start with the recommended interviewer pattern."))
            .andExpect(jsonPath("$.replayReadiness.primaryCtaLabel").value("Start replay"))
            ...
            .andExpect(jsonPath("$.actionRecommendations.primaryActionLabel").value("Start replay"))
            .andExpect(jsonPath("$.actionRecommendations.primaryActionTarget").value("review:replay"))
            .andExpect(jsonPath("$.actionRecommendations.primaryActionTargetPayload.focus").value("recommended_replay_mode"))
            .andExpect(jsonPath("$.actionRecommendations.availableActionLabels.review_answers").value("Review answers"))
            .andExpect(jsonPath("$.actionRecommendations.availableActionTargets.review_answers").value("question:questions"))
            .andExpect(jsonPath("$.actionRecommendations.blockingReasons.length()").value(0))
            .andExpect(jsonPath("$.actionRecommendations.blockingReasonDetails.length()").value(0))
            .andExpect(jsonPath("$.actionRecommendations.canConfirm").value(false))
            .andExpect(jsonPath("$.replayLaunchPreset.sourceInterviewRecordId").value(recordId))
